...
export default function LessonScreen() {
  ...
  // Add this: Refresh progress when screen comes into focus
  useFocusEffect(
    useCallback(() => {
      if (sections.length > 0) {
        loadSectionProgress(sections);
      }
    }, [sections, lessonId])
  );
  ...
  const loadLessonData = async () => {
    ...
    try {
      ...
      const itemsByType = (it || []).reduce((acc: { [key: string]: ItemRow[] }, item: ItemRow) => {
        if (!acc[item.kind]) acc[item.kind] = [];
        acc[item.kind].push(item);
        return acc;
      }, {});

      // Define learning sequence and section properties
      const sectionOrder = [
        { type: "vocab", title: "Vocabulary", icon: "📚", color: "#f3e5f5", borderColor: "#9C27B0" },
        { type: "sentence", title: "Sentences", icon: "📝", color: "#e3f2fd", borderColor: "#2196F3" },
        { type: "picture-match", title: "Picture Match", icon: "🖼️", color: "#e8f5e8", borderColor: "#4CAF50" },
        { type: "tile-build", title: "Tile Builder", icon: "🧩", color: "#fff3e0", borderColor: "#FF9800" },
      ];
  ...
  const loadSectionProgress = async (sections: ItemSection[]) => {
    try {
      ...
      console.log("Completed sections from DB:", progress?.last_position?.completed_sections);

      if (progress?.last_position?.completed_sections) {
        setCompletedSections(new Set(progress.last_position.completed_sections));
      }
    } catch (error) {
  ...
  const markSectionComplete = async (sectionType: string) => {
    const newCompleted = new Set([...completedSections, sectionType]);
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, gap: 16 }}>
        ...
        <View style={{
          ...
          <Text style={{
            ...
            {Math.round((completedSections.size / sections.length) * 100)}% Complete
          </Text>
        </View>
        ...
        {sections.map((section, index) => {
          ...
          const isPrevious = index > 0 && !completedSections.has(sections[index - 1].type);
              ...
              <View style={{ padding: 16 }}>
                {isLocked ? (
                  ...
                  <View style={{ gap: 16 }}>
                    ...
                    <Pressable
                      onPress={() => router.push(`/(student)/lesson/section?id=${lessonId}&type=${section.type}&from=lesson`)}
                      style={{
                        backgroundColor: section.borderColor,
                        padding: 16,
                        borderRadius: 12,
