...
export default function Lessons() {
  ...
  const auth = useAuth();
  // Set loading based on store data availability
  const loading = lessons.lessons.length === 0 || Object.keys(lessons.itemCounts).length === 0;
  const [err, setErr] = useState<string | null>(null);
  const [expandedLessons, setExpandedLessons] = useState<Set<string>>(
    new Set()
  );

  const lessonUnits: LessonUnit[] = (lessons.lessons || []).map((lesson) => {
    const lessonItemCounts = lessons.itemCounts[lesson.id] || {};
    const progress = lessons.progress[lesson.id];

    const sections = [
      {
        type: "vocab",
        count: lessonItemCounts["vocab"] || 0,
      },
      {
        type: "sentence",
        count: lessonItemCounts["sentence"] || 0,
      },
      {
        type: "picture-match",
        count: lessonItemCounts["picture-match"] || 0,
      },
      {
        type: "tile-build",
        count: lessonItemCounts["tile-build"] || 0,
      },
    ]
    ...
    return {
      ...
      unit_title:
        (lesson.units as any)?.title || `Unit ${lesson.unit_id.slice(0, 8)}`,
      sections,
      progress: {
  ...
  // Compute course title directly from store data
  const courseTitle = (() => {
    if (lessons.lessons && lessons.lessons.length > 0) {
      const firstLesson = lessons.lessons[0];
      const courseData = (firstLesson.units as any)?.courses;
      if (courseData?.title) return courseData.title;
    }
    return "Your Lessons";
  })();
  ...
  const handleRefresh = async () => {
    ...
    // Data updates automatically from store - no need to manually process
  };
